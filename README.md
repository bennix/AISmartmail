...
- Gmail setup helper: users can choose Gmail, use built-in IMAP/SMTP settings, and follow in-app guidance to create a Google App Password when OAuth is not used.
...
- Gmail 配置助手：用户可选择 Gmail，应用会内置 IMAP/SMTP 配置，并在应用内指导用户获取 Google 应用专用密码。
